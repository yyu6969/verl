...
class GPUComputeMonitor:
    ...
    def record_prefill(self, prompt_id, model, inputs):
        """
        Record GPU utilization for the 'prefill' phase, and store the prompt token length.
        """
    ...
    def record_decode(self, prompt_id, model, inputs, max_new_tokens=50):
        """
        Record GPU utilization for the 'decode' phase, 
        and store the actual generation tokens in self.decoded_outputs.
        """
        # We'll capture the return value from model.generate in a local variable:
        container = {}

        def decode_func():
            # The actual generation
            outputs = model.generate(**inputs, max_new_tokens=max_new_tokens)
            container['outputs'] = outputs  # store so we can retrieve after monitoring

        data = self._run_with_monitoring(decode_func)
        self.decode_data[prompt_id] = data

        # Save the generated token IDs for later decoding
        self.decoded_outputs[prompt_id] = container['outputs']

    def plot_time_utilization(self, output_dir="."):
        """
        For each prompt_id, create a single chart that shows
        both prefill and decode utilization as two lines on the same timeline,
        and print out token length, plus prefill/decode durations.

        Returns a list of (prompt_id, figure_path) so we can log them to wandb if desired.
        """
        ...
        for pid in sorted(all_prompt_ids):
            ...
            ax.set_title(f"GPU Utilization - {pid} - {token_len} tokens")
...
def run_compute_utilization_test(model_path, 
    ...
    monitor = GPUComputeMonitor(device_index=0, poll_interval=0.01)
